...
// Cube represents a hex coordinate in the cube coordinate system
type Cube [3]int
...
// NewCube creates a new cube coordinate with the given component values
func NewCube(q int, r int, s int) Cube {
	check.That(q+r+s == 0, "cube coordinates (q + r + s) must equal 0")
	return Cube{q, r, s}
}

// ZeroCube creates a new cube coordinate with the all zero component values
func ZeroCube() Cube {
